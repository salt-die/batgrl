...
from nurses_2.colors import ColorPair, RED, BLACK, gradient, lerp_colors
...
TRANSITIONS = np.array([.07, .004, 1e-5])


class Bleed(TextWidget):
    def __init__(self, banner, button, blood_color_pair: ColorPair=RED_ON_BLACK, **kwargs):
    ...
    async def _drip(self):
        ...
        weights = weights = np.array([.0, .89, .1])
...
class TextButton(ButtonBehavior, TextWidget):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._dark_red = lerp_colors(RED, BLACK, .35)
    ...
    async def _to_red(self):
        ...
        while self._i < len(self._grad) - 1:
            self._i += 1
            if not self.state is ButtonState.DOWN:
                self.colors[..., :3] = self._grad[self._i]
...
class DripApp(App):
    async def on_start(self):
        ...
        bleed = Bleed(banner, button, size=(h + 9, w), pos_hint=(.5, .5), anchor="center")
