...
Some time between numpy v2.0.0 and v2.3.1, numpy started casting IntFlag to int64. This
prevents using IntFlag in bitwise operations with uint arrays. So, to remedy this, this
module defines an IntFlag-like enum using numpy dtypes instead. ``Style`` then uses this
enum so that bitwise operations on uint arrays work as expected, in particular, with the
``"style"`` field of ``Text.canvas``.
"""
