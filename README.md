...
Next, let's add paddles for each player. We can respond to key presses by implementing a widget's `on_key_press` method. Player 1's paddle can be moved up and down with `w` and `s` and Player 2's paddle can be moved up and down with `up` and `down`:
...
We should add a ball. For the ball to bounce off the top and bottom of the play field and paddles we'll add a few math-y methods. To keep the ball moving we can schedule an `asyncio.Task` in the `on_add` method. `on_add` is called when the widget is added to the widget tree. Similarly, `on_remove` is called when the widget is removed from the widget tree. We should cancel any tasks we created in `on_add` in `on_remove`:
